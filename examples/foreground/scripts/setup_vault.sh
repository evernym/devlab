...
if [ -f /devlab/persistent_data/vault-example/env ] ; then
...
vault operator init -key-shares=1 -key-threshold=1 2>&1 > /devlab/persistent_data/vault-example/init.out

echo "Parsing unseal and root tokens"
export UNSEAL_TOKEN=$(cat /devlab/persistent_data/vault-example/init.out | grep '^Unseal Key 1: ' | awk '{print $4}')
export VAULT_TOKEN=$(cat /devlab/persistent_data/vault-example/init.out | grep '^Initial Root Token: ' | awk '{print $4}')
...
cat <<EOF > /devlab/persistent_data/vault-example/env
...
vault token create -policy=my_app -no-default-policy -period=336h 2>&1 > /devlab/persistent_data/vault-example/my_app_token.out
export MY_APP_TOKEN=$(cat /devlab/persistent_data/vault-example/my_app_token.out | grep '^token ' | awk '{print $2}')

echo "Parsing and storing my_app token into env file"
echo "export MY_APP_TOKEN='${MY_APP_TOKEN}'" >> /devlab/persistent_data/vault-example/env
